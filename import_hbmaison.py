...
def run_import(excel_path: str, app_label: str = "ledger", limit: Optional[int] = None, strict: bool = False):
    ...
    def get_or_create_item(name: Optional[str]):
        if not name: return None
    ...
    def do_import():
        ...
        for _, r in df.iterrows():
            ...
            item = get_or_create_item(r.get("item_name")) if r.get("item_name") else None
            ...
            if op_type in (OP_SELL, OP_BUY, OP_USE):
                if not item:
                    item, _ = Item.objects.get_or_create(name="(سیستمی) قلم نامشخص")
                if not qty or qty <= 0:
                    continue
